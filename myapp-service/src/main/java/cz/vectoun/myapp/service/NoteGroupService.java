package cz.vectoun.myapp.service;

import cz.vectoun.myapp.persistance.entity.NoteGroup;
import cz.vectoun.myapp.persistance.entity.User;

import java.util.List;

/**
 * @author Vojtech Sassmann <vojtech.sassmann@gmail.com>
 */
public interface NoteGroupService  extends Service<NoteGroup> {

    /**
     * Creates new note group for given user
     *
     * @param noteGroup noteGroup to be created
     * @param user owner of the newly created note group
     */
    void createNoteGroup(NoteGroup noteGroup, User user);

    /**
     * Finds all note groups for given user.
     *
     * @param user user
     * @return list of all note groups for given user
     */
    List<NoteGroup> findAllForUser(User user);
}
